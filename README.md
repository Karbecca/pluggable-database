...
The first step is to create a new user with administrative privileges. The following query creates the user re_plsqlauca and grants all necessary privileges:

```sql
CREATE USER re_plsqlauca IDENTIFIED BY auca;
GRANT ALL PRIVILEGES TO re_plsqlauca;
```
- ``re_plsqlauca``: The new user’s name
...
```
- ``re_to_delete_pdb``: The name of the PDB to be created.
- ``ADMIN USER re_plsqlauca``: The admin user for the new PDB.
- ``FILE_NAME_CONVERT``: Specifies the conversion of file paths from the PDB seed (pdbseed) to the new PDB's directory.
![Creating PDB](create%20puggable%20db.png)
...
```
![Connecting to PDB](connect%20pbdb.png)
## 4. Deleting the PDB
...
```
![connecting to PDB](delete%20pbdb.png)
### Step 5: Oracle Enterprise Manager
![Connecting to PDB](oracle.png)